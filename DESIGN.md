...
**Le disque remplit son bloc.** La zone du chrono occupe toute la largeur de la colonne centrale — **le retrait de la colonne est annulé pour elle**, l'anneau va d'un bord à l'autre — et les trois éléments de la colonne (REP, chrono, `PASSER LE TOUR`) se resserrent d'une unité au lieu de trois, pour lui rendre la hauteur. C'est cette hauteur qui gouverne sa taille à 1920×1080, la largeur qui la gouverne sur tablette.

**La gouttière entre les trois colonnes est UNE SEULE VALEUR** (`--game-column-gutter`, frontmatter `spacing` › `column-gutter`), **à une unité de grille** — 8 px sur tablette, 13 px à 1920 (Story 11.5, Tasks 2 et 3, 2026-09-18 ; introduite à `0` par le découplage, puis arbitrée au rendu par Nathan). Arbitrage pris explicitement : c'est un **token**, pas une constante d'écran transmise en prop. Raison — c'est exactement le cas qui a justifié `--game-clock-bleed` (`CLAUDE.md` §10) : **trois lectures dans trois fichiers qui doivent coïncider au pixel**, et une gouttière est un **rythme de mise en page**, pas une position d'écran (ce qui distinguait les quatre faux tokens d'inset rapatriés en 11.3). Les trois lectures :
- `GameView` — l'écart réel entre les colonnes (`gap` de la rangée) ;
- `CenterPanel` — la zone du chrono s'élargit de `gouttière + débordement` de chaque côté (plus son propre padding), pour **traverser** la gouttière et déborder encore dans la carte ;
- `PlayerPanel` — la gouttière intérieure reste `≥ débordement` seul, et **ne suit pas** la gouttière entre colonnes : la part qui tombe dans la gouttière tombe **hors** de la carte, elle n'a rien à réserver.

⚠️ **`ShotClock` ne lit PAS la gouttière** : depuis l'abandon du débordement, il n'a plus rien à caler dessus — il remplit la colonne, et la colonne est ce que la gouttière écarte. Une lecture de plus serait une lecture à tenir pour rien.

⚠️ **Plafond mesuré de la gouttière** (1920×1080, 2026-09-18) : à ce format la **hauteur** gouverne la taille du disque (427 px de disque pour 462 de zone), si bien que le disque ne mord la carte que de **21 px** — et une gouttière les consomme un pour un. Au-delà de ~21 px à 1920, le disque **se décolle des cartes** : le débordement du chrono n'existe plus. Toute proposition de découpe se mesure contre ce plafond. Valeurs mesurées (morsure du disque dans chaque carte) : gouttière 0 → 21,5 px · **1 unité, retenue → 8,5 px** · 16 px → 5,5 px · 21 px → 0,5 px · **2 unités → décollé**. C'est ce plafond qui **interdit une deuxième unité** de gouttière, pas un avis : une gouttière de 2 unités détache le chrono des cartes à 1920.
...
- **Conteneurs à angles vifs** (aucun token, aucun `rounded-*`) : tuiles, barre latérale, conteneur et cellules du récap, bandeaux. « L'app doit se lire comme un outil tactile, pas comme une app mobile » (Nathan, 10.1). ⚠️ **Le scoreboard fait exception depuis la Story 11.5** — voir ci-dessous ; la règle vaut intacte partout ailleurs.
...
- **Ce que l'exception NE dit PAS.** Elle ne rouvre ni les tuiles, ni la barre latérale, ni le récap, ni les bandeaux : ils restent à angles vifs. ⚠️ **Étendue le 2026-09-19** (Nathan, au rendu) : elle ne vaut plus pour le seul scoreboard mais pour **la mise en page carte · colonne · carte**, qu'il partage avec le **paramétrage des joueurs**. C'est cette anatomie qui porte les blocs arrondis, pas un écran nommé — un troisième écran qui l'adopterait en hériterait sans nouvelle exception.
- **Ce que l'exception coûte, mesuré et accepté.** La gouttière d'une unité réduit le débordement du disque du chrono dans les cartes à **1920×1080** : de 21,5 px à **8,5 px** (voir Layout › plafond mesuré). Aux deux formats tablette, rien ne change — la largeur gouverne, le disque grandit avec sa zone. Accepté au rendu.
