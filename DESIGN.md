...
components:
  ...
  button-setup:
    backgroundColor: "{colors.bleu-roi}"
    textColor: "{colors.blanc-pur}"
    typography: "{typography.cta-narrow}"
    rounded: "{rounded.tappable}"
    shadow: "{shadows.cta-relief-blue}"
    height: "{spacing.touch-target}"
    padding: "0 16px"
...
### Tertiary
- **Jaune franc** (#FFF200, teinte 57°) : carte du joueur de droite, bille jaune. Bleu à zéro, saturation pleine : c'est ce qui le rend franc plutôt que doré ou délavé (quatre essais en 10.4). Ne jamais y remettre de bleu « pour adoucir ». Encre noire 17,95:1 (calculé sur l'aplat plein). ⚠️ **Remonté de #FFE000 le 2026-09-19** (Nathan, au rendu, Story 11.5 : « un jaune plus peps ») : même principe — bleu à zéro —, teinte poussée vers le citron (52,7° → 57°) et plus lumineux. Écartés au même rendu : les jaunes chauds (#FFCF33, retour au doré), beurre (#FFE566) et fluo (#F5FF00, reflet vert), et les versions plus claires de celui-ci (#FFF533 à #FFF880 : dès #FFF766, il pâlit à côté de la carte blanche).
- **Jaune bandeau** (#E6DA00) : bandeau haut de la carte jaune (nom, distance, restant, stats), le jaune franc à 90 %. Encre noire 14,39:1 (calculé sur l'aplat plein).
...
- **Marine clair** (#272E49, `--color-bg-raised`) — **niveau 1**, un cran au-dessus : fond des quatre écrans hors jeu (accueil, sélection JDS, paramétrage, récap ; le scoreboard n'en a pas, ses colonnes couvrent l'écran) et **carte de pop-up en verre dense** (95 % d'opacité, flou de 16 px de ce qui passe sous la carte seule — Story 11.5 ; le voile plein écran reste sans flou). Blanc dessus 13,35:1 sur l'aplat plein ; **11,47:1 au pire** à travers le verre (calculé : 95 % de `#272E49` composé sur un aplat **blanc pur** dessous, le cas le plus clair possible — 11,78:1 sur le jaune de carte ; le verre léger à 88 % descendait à 9,16:1). Le flou ne peut qu'éloigner la valeur de ce pire cas : il mélange le blanc avec ce qui l'entoure.
...
- **Stat** (`stat` ; 700, `clamp(14px, 1.2vw, 23px)` → 14 / 14 / 23, majuscules, interlettré `stat` sur les libellés) : `MOY` / `SÉRIE` (libellé à 60 %, valeur en 900), `REP`, libellés du récap (blanc 50 %), mode du récap, `BILLE BLANCHE`, `NOM` / `DISTANCE` et leurs placeholders, badge `BIENTÔT` de tuile. « Grand texte » (≥ 18,66 px en 700, seuil 3:1) seulement au-delà de 1555 px de large : sur tablette, tout ce qui est en `stat` sur un dégradé doit tenir 4,5:1.
- **CTA étroit** (`cta-narrow` ; 900, `clamp(14px, 1.2vw, 23px)` → 14 / 14 / 23, majuscules, interlettré `label`) : la taille de `stat` avec la graisse et l'interlettrage des CTA — libellé de `PASSER LE TOUR` et des deux CTA de réglage, sur une ligne. Rôle à part depuis la revue de la 11.5 (2026-09-19) : les trois renvoyaient à `stat` (700, interlettré `stat`) et le frontmatter décrivait un autre texte que celui rendu. Jamais « grand texte » sur tablette : 4,5:1 exigé, tenu par la position du libellé (Components › Buttons).
...
Le relief est réservé à **ce qui se tape**, à **ce qui flotte**, à **la barre latérale** (Story 11.2, modèle Cueuny : c'est son ombre qui la sépare du fond, plus un filet) et à **la barre basse du scoreboard** (Story 11.5, « O2 » : ombre projetée vers le haut sur le grand bloc). Chaque ombre est un token `--shadow-<nom>` de `main.css` (frontmatter `shadows`), consommé par `shadow-<nom>` ; aucune ombre ne s'écrit en valeur arbitraire dans un gabarit :
...
**Pictos** : Lucide, viewBox 24, trait 2 px arrondi, sans remplissage, SVG inline, `currentColor`. Tailles : 32 px (`size-4`) dans la barre latérale et la barre basse, 32 px (`size-4`) au-dessus du libellé des CTA de colonne étroite (`PASSER LE TOUR`, réglages du paramétrage, Story 11.5), 40 px (`size-5`) pour la flèche de tuile et le chevron de `DÉMARRER`.
...
- **CTA de colonne étroite** (`setup`, `pass`) : **picto au-dessus**, libellé en `cta-narrow` (taille de `stat`, 900, **interlettré `label`**), **sur une ligne** (vérifié à 1133, la colonne la plus étroite). `CHANGER DE BILLE` mettait son picto en ligne et passait sur deux lignes à côté d'un `PASSER LE TOUR` de même largeur qui mettait le sien au-dessus.
...
- **Tuile de mode** : dégradé bleu (ou, si `BIENTÔT`, bleu sombre à 45 % **désaturé** et titre en blanc à 50 % — Règle de l'inactif, Story 11.5), angles vifs, `p-2`, titre `title` 900 en haut à gauche, flèche nue 40 px en bas à droite ; jointive, séparée par un filet.
...
- **Conteneur de récap** : surface voilée, filet, bandeau clair échancré en tête, cellules à `gap-1` où la couleur est portée par la cellule (rouge profond pour la victoire / surface voilée pour le reste — la même `bg-surface` que le conteneur), colonne de libellés en creux, blanc 75 % (Story 11.5, voir Layout › Récap).
...
### Pop-ups (signature)
Un seul patron pour les quatre (`PromptModal`, `ScoreEntryDock`, `NumericPadDock`, `AlphaKeyboardSheet`), et depuis la Story 11.3 ce patron **est un composant** — `PopupCard.vue` (contrat en fin de section) : voile noir 25 % sans flou, `role="dialog"`, carte en verre dense (Story 11.5, « V2 ») à rayon pop-up et filet 22 %, pied de CTA en `gap-2`. **Jamais de croix** : le retour est un gros CTA `ANNULER`, et toute pop-up qui en porte un se ferme aussi au tap dehors (geste complet, appui et relâchement du même pointeur). Une pop-up sans retour (fin de partie par série) garde un voile inerte. Titre toujours centré ; pas de message redondant.
...
### Do:
- **Do** poser toute nouvelle commande sur l'une des six variantes de `CtaButton` (accent, neutre, réglage, `DÉMARRER`, barre basse, `PASSER LE TOUR`), au format que sa largeur lui donne, plutôt qu'en créer une septième ; la passe a pour critère qu'un écran de plus n'ajoute ni composant de base ni token.
...
- **Do** reprendre la coupe en biais, les deux niveaux de marine et le filet à 22 % comme vocabulaire de tout nouvel écran ; angles vifs sur les conteneurs, sauf la mise en page carte · colonne · carte (zone 16 px, blocs 12 px), 8 px sur tout ce qui se tape, 12 px sur une carte de pop-up.
