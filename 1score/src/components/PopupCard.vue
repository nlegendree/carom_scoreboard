...
const props = withDefaults(
  defineProps<{
    ...
    testid: string
    cardTestid: string
    // Nom accessible, EXCLUSIFS l'un de l'autre : `labelledBy` pointe le titre VISIBLE
    // (`PromptModal` est la seule des quatre à en avoir un, et son `id` vient de `useId()`) ;
    // `label` est l'`aria-label` statique des trois hôtes de saisie, qui n'ont pas de titre.
    labelledBy?: string
    label?: string
    ...
    // Placement (AC4 de la 11.3) : le côté où la carte se pose, et la bande à réserver du
    // côté OPPOSÉ. L'hôte les calcule à partir de SA mise en page — `PopupCard` ne sait rien
    // de la géométrie d'un écran.
    align?: TableSide
    reserve?: string
  }>(),
  {
...
const placement = computed(() => {
  ...
  const reserve = props.reserve ?? GUTTER
  return props.align === 'left'
    ? { paddingInlineStart: GUTTER, paddingInlineEnd: reserve }
    : { paddingInlineStart: reserve, paddingInlineEnd: GUTTER }
})
</script>
