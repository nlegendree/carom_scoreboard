...
⚠️ ~~**À savoir pour la passe contraste AA (Story 10.7)**~~ — **TRANCHÉ le 2026-09-14 par la Story 10.7 : le ruban est assombri à `#D0343F`, le blanc y tient 4,95:1 et le noir tombe à 4,24:1. Le blanc est désormais le PLUS contrasté des deux, et la marge ne dépend plus de la taille des valeurs. État d'avant la correction ci-dessous, pour mémoire.** Sur `--color-victory-ribbon` (#E63946), le blanc donne **4,17:1** et le noir **5,04:1** — le blanc est donc le choix le MOINS contrasté des deux. Il passe AA pour les tailles employées ici (tout est en `text-label`, ≥ 24 px et gras, donc « grand texte », seuil 3:1) mais **pas** le seuil 4,5:1 du texte courant. Deux conséquences à retenir : ne pas descendre la taille de ces valeurs sans revoir la teinte, et si le seuil 4,5 est visé, assombrir le ruban vers `--color-brand-red` (#D0343F) porterait le blanc à 4,95:1. Consigné dans `main.css` au-dessus du token.

*Livré en Story 10.7 (2026-09-14)* — **finition transverse, dernière story de l'epic**. Aucun écran ne change de forme, **aucun fichier créé ni supprimé**, `git diff --stat src/stores/` **vide**. Trois chantiers indépendants :

- **Sémantique de dialogue sur les QUATRE pop-ups.** `PromptModal`, `NumericPadDock`, `AlphaKeyboardSheet` et `ScoreEntryDock` portent `role="dialog"` + `aria-modal="true"` sur leur racine. ⚠️ **Quatre et non trois** : l'AC d'`epics.md` a été écrite avant que la 10.4 remplace `ScoreEntryModal` par `ScoreEntryDock`. Seul `PromptModal` a un titre visible : il se nomme par `aria-labelledby` vers l'`id` de son `<h2>`, produit par **`useId()`** (Vue 3.5) et donc unique par instance — un `id` littéral collisionnerait dès que deux pop-ups coexistent, et une collision casse `aria-labelledby` silencieusement. Les trois autres prennent un `aria-label` statique (`Réglage de la distance`, `Saisie du nom`, `Saisie du score`). **Rien d'autre** : pas de piège à focus, de `tabindex`, d'écoute de `Escape` ni d'`inert` — l'arbitrage « borne fixe » est inchangé. Le voile garde ses handlers pointer **sans** `role="button"` : il porte déjà `role="dialog"`, et un élément n'a qu'un rôle. Exception consignée dans `CLAUDE.md` §2 (ce que DT3 demandait) ; **DT3 est CLOSE**.
- **Garde `prefers-reduced-motion` globale** dans `main.css`, après les autres règles globales. Elle coupe `animate-input-reject` (pulsation de refus de plafond) et `animate-input-flash` (flash de frappe), **deux retours de confort** que l'haptique et l'affichage du chiffre redisent déjà. Elle **épargne** le fondu du chrono (trois `transition-*` de `ShotClock`) **et** la barre de rebours (`animate-input-countdown`) : *pourquoi* — toutes deux portent une **information** qu'aucun autre élément ne donne, le temps restant et le fait que le score va se valider seul **et changer le tour**. Le rebours est un **écart assumé** à la lettre de l'AC, qui n'exemptait que le chrono (décision de Nathan, 2026-09-14). ⚠️ **Deux pièges figés ici** : (1) `animate-input-flash` va de `opacity: 1` à `0` en `forwards` et son élément est un voile noir posé **par-dessus** la valeur de série — `animation: none` seul le laisserait peint **en permanence**, la garde pose donc aussi `opacity: 0` ; (2) **aucune garde attrape-tout** (`*, *::before, *::after { animation-duration: 0.01ms }`), qui emporterait précisément les deux animations conservées. Règle de maintenance dans `CLAUDE.md` §11.
- **Passe contraste AA mesurée**, élargie à tous les textes translucides du produit (décision de Nathan) et non aux seules cibles de l'AC. **Trois échecs, trois corrections** : `--color-victory-ribbon` **`#E63946` → `#D0343F`** (blanc 4,17 → **4,95:1**, conforme même au seuil 4,5:1 du texte courant — la dette de la 3e passe de la 10.5 est close **par la couleur**) ; barre de rebours de `ScoreEntryDock` `bg-white/70` → `bg-white` (2,45 → **3,46:1** ; c'est un objet graphique porteur d'information, WCAG 1.4.11, seuil 3:1) ; placeholders de `PlayerSetupCard` `opacity-55` → `opacity-60` (4,20 → **4,98:1** sur la carte jaune — ⚠️ mesurés sur la **box du champ** `bg-black/8`, pas sur la carte nue, ce qui les rendait plus sombres que la table de la story ne le supposait). Un quatrième reste partiellement ouvert, consigné dans `deferred-work.md` : l'accroche de `ModeTile` passe de `text-white/80` à `text-white` (2,78 → **3,46:1**) mais reste sous 4,5:1 à sa taille `text-stat` — **aucune tuile ne l'affiche**, la prop est gardée au contrat, et rouvrir les accroches obligera à en remonter la taille. ⚠️ **`--color-victory-ribbon` et `--color-brand-red` valent désormais la même couleur et restent DEUX tokens** : deux rôles qui peuvent rediverger, et Nathan a demandé un design system des couleurs plus tard, hors Epic 10. Un cas de `main.css.test.ts` verrouille la non-fusion.
- **Outillage de test** : `vitest.config.ts` gagne `test.css: { include: [/main\.css/] }`. Vitest remplace par défaut **tout** import CSS par une chaîne vide, **`?raw` compris** — le motif employé partout ailleurs pour lire une source rendait ici `''`, et un test de garde CSS serait passé en ne vérifiant rien. `main.css` est le seul CSS lu par un test.
