...
**PromptModal** *(ajouté le 2026-09-10, Story 1.10)* — *(2026-09-11, Epic 10 : variante « liste » à *n* CTA pour le choix Cadre ; usage « FERMER 1SCORE ? » ; sémantique de dialogue et garde reduced-motion, Story 10.7.)*
...
- Sidebar réduite (`RETOUR`). Zone principale : titre `JEUX DE SÉRIES` en display, puis **4 tuiles** `LIBRE` · `BANDE` · `CADRE` · `4 BILLES`, même gabarit que l'accueil, toutes en `--color-tile-jds` avec une accroche courte (« Sans contrainte », « Une bande avant le second point », « 47/2 · 47/1 · 71/2 », « Deux billes rouges »).
...
- **Saisie de la distance** : le tap sur `DISTANCE` ouvre le **pavé numérique nu** (`NumericPad` sans carte ni en-tête) **dans la colonne centrale**, à la place des CTA de réglage ; le reste de l'écran passe sous un **voile léger flouté** (blur 4 px, 30 % noir), **sauf la carte visée**, qui reste nette et dont le champ `DISTANCE` **s'actualise à chaque touche**. Sous le pavé, un `VALIDER` accent pleine largeur de colonne ; `AC`/`C`, `⌫` inchangés. Fermeture par `VALIDER`, par la croix en haut du dock, ou par un **geste complet** sur le voile (appui **et** relâchement — règle ScoreEntryModal reprise) : les deux dernières abandonnent la saisie et restaurent la valeur précédente. Plafond 3 chiffres : frappe ignorée avec pulsation (inchangé). **Le plafond est unique** (`MAX_TARGET_SCORE`, dette 10.7) et redescend vers le pavé.
- **Saisie du nom** : le tap sur `NOM` ouvre l'`AlphaKeyboard` en **bandeau bas pleine largeur** (hauteur ≈ 45 % de l'écran, conteneur à contour, croix à gauche, `VALIDER` à droite), même voile flouté, la carte visée nette et le champ `NOM` s'actualisant en direct — le clavier à 10 colonnes ne tient pas dans la colonne centrale, et la pop-up disparaît de toute façon. Clavier **complété** : tiret, apostrophe, `Ë Ï Î Ô Û` (dette reprise). Touches ≥ 57 px (exception UX-DR8 des claviers intégrés, inchangée).
