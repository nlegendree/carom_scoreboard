...
*Section réécrite en **intentions** le 2026-09-15 (purge, Story 11.0) : toute valeur — couleur, taille, espacement, rayon — vit dans `DESIGN.md` (source unique du visuel, décision de Nathan du 2026-09-15) et n'est plus redite ici. L'extraction de première passe du 2026-09-08 (captures CUESCO/Billiboard) est dans l'historique git.*

### Color System

**Intention : une salle noire où seules les deux cartes joueur sont en pleine lumière.** Fond sombre, quasi noir ; carte de gauche blanche, carte de droite jaune, encre noire sur les deux — la couleur suit la bille, fixée au côté *(révisé le 2026-09-08 ; depuis l'Epic 10, `CHANGER DE BILLE` peut mettre la carte jaune à gauche, la couleur suit toujours la bille — voir §10.3)*. Le joueur dont c'est le tour est encadré d'un liseré rouge : c'est la **présence** du cadre, et non sa teinte, qui porte l'information (UX-DR14). Un seul bleu, celui du drap, pour tout ce qui se règle ou se saisit ; un rouge pour ce qui engage la partie, la marque et la victoire ; un rouge d'alerte réservé au chrono. Valeurs, dégradés et règles nommées (bille, bleu unique, rouge qui engage, voile clair) : `DESIGN.md` › Colors.

### Typography System

**Intention : le chiffre de score domine tout**, en graisse maximale et chiffres tabulaires ; libellés en majuscules, nettement plus petits ; une seule famille — la police système en attendant une police display (dette consignée, décision Nathan 10.1). L'échelle est fluide, lisible à 2 m (NFR10) et 5 m en vue salle V2+ (NFR11), et doit être **conçue pour 1920×1080**, format de référence (Story 11.1). Échelle, graisses, interlettrages : `DESIGN.md` › Typography.

### Spacing & Layout Foundation

**Intention : la densité vient de la taille des éléments, pas de leur nombre.** Les blocs joueur remplissent leur colonne sans marge décorative ; éléments jointifs séparés par des filets, angles vifs (Epic 10). Zones tactiles au plancher NFR9 (≥ 90×90 px) sur toute commande de jeu, hors claviers dessinés (exception UX-DR8). Grille, rythme, largeurs de colonnes : `DESIGN.md` › Layout.
...
- *Accessibilité* : plancher des **claviers intégrés** (exception UX-DR8, valeur dans `DESIGN.md` › Keys), et non les 90px des zones de commande : trois colonnes à 90px exigent une largeur que le panneau ne peut pas offrir en portrait. Retour haptique + visuel <100ms (NFR1), porté par le composant hôte.
...
**Paysage exclusivement** (décision de Nathan, 2026-09-11) : ni téléphone, ni portrait. Trois formats de vérification — 1920×1080 (référence : écran 21,5″ tactile de club), 1180×733 (format de travail réel : iPad en Sidecar sous Chrome), un petit iPad en plancher. Une seule mise en page à trois colonnes ; les variantes des cartes joueur se font en container queries, jamais en breakpoints d'écran. Détail : `DESIGN.md` › Layout ; contexte d'usage : `PRODUCT.md`.

### Breakpoint Strategy

Aucun breakpoint d'écran n'est employé par l'interface (paysage seul). La règle « mobile-first, trois breakpoints » héritée de l'architecture (AR19) est caduque pour l'UI ; `CLAUDE.md` §8 est à réécrire en ce sens avec la Story 11.3.
...
> **Statut de cette section (2026-09-15, purge — Story 11.0).** Elle est le **journal d'intention et de décisions de l'Epic 10**, livré et revu. Les valeurs chiffrées (px, %, hex) qui subsistent dans ses notes de rendu datées sont **historiques et non normatives** : la source unique du visuel est `DESIGN.md`, extrait du code livré. Les paragraphes normatifs (10.1, 10.2, 10.3 hors notes de rendu, 10.6, 10.7) ont été vidés de leurs valeurs au profit d'intentions ; en cas d'écart entre une valeur restée ici et `DESIGN.md`, `DESIGN.md` a raison.
...
Les tokens de l'epic (bleu du drap, surface voilée, filets, rayons, dégradés de tuiles et de fond, bandeaux de carte, rouge de marque) et leurs valeurs vivent dans `DESIGN.md` (frontmatter, sections Colors et Shapes) ; le tableau de valeurs indicatives du 2026-09-11 est retiré le 2026-09-15 (Story 11.0). Décisions datées conservées : le bleu du drap est **mesuré** sur `simonis-prestige.gif` le 2026-09-11 (Story 10.1) ; le bandeau de la carte jaune est figé à la passe contraste de la 10.7 ; les tuiles vert / orange / violet sont remplacées par des dégradés de bleu (amendement du 2026-09-11) ; le bleu drap n'est **jamais** posé seul sur un bouton.

**Typographie** : accroche d'accueil et mot `1Score` en display ; libellés de picto en majuscules, sans interlettrage (passe de rendu de la 10.1). Police display non choisie : police système (décision de Nathan, 10.1).

**Pictos** : jeu unique, trait fin uniforme, sans remplissage, SVG inline (aucune police d'icônes). Table de correspondance en 10.7 ; tailles dans `DESIGN.md` › Shapes.

### 10.2 — Barre latérale contextuelle (`SideBar`)

Nouveau primitif de coquille d'écran, modèle Cueuny : colonne **gauche**, largeur fixe, sur toute la hauteur de l'écran, en aplat collé au bord depuis la passe de rendu de la 10.1 (dimensions et couleurs : `DESIGN.md` › Navigation ; le portrait est caduc).

- **En-tête** : logo et le mot `1Score` dessous, centrés, sur un bandeau rouge coupé en biais (10.1). Le tap sur l'en-tête est **sans effet** (ce n'est pas un bouton « accueil » : sur un écran de jeu on ne doit pas pouvoir quitter par accident, et hors jeu le retour existe déjà).
- **Items** : empilés sous l'en-tête, picto au-dessus d'un libellé court, zone tactile au plancher NFR9, pleine largeur de la colonne. Un item de **sortie** (croix, « Fermer l'application », « Quitter ») est toujours **calé en bas** de la colonne, isolé du reste — l'action la plus irréversible est la plus éloignée du geste courant.
...
- **Accroche** : une phrase courte en police display (échelle `hero` de `DESIGN.md`), blanc sur le dégradé, alignée à gauche (ex. « À vous de jouer. » — libellé final à choisir par Nathan ; pas de sous-texte). Cette zone accueillera plus tard l'identité du club et l'emplacement image. L'accueil reste **l'écran de veille** : rien n'y bouge, rien n'y clignote.
- **Tuiles de mode** : une rangée basse de **4 tuiles** de hauteur égale (proportion et plancher dans `DESIGN.md` › Layout), conteneurs à contour, fond dans leur couleur (`--color-tile-*`) ~~à 85 % d'opacité~~ sur le dégradé *(**caduc** depuis la passe de rendu de la 10.1 : les tuiles sont des dégradés PLEINS, plus aucune n'est à 85 % — seules les tuiles `BIENTÔT` sont atténuées, à 45 %. Constaté à la passe contraste de la 10.7.)* : `3 BANDES` · `JEUX DE SÉRIES` · `QUILLES` · `CASIN`. Chaque tuile porte : titre (échelle `title`), **accroche** d'une ligne en dessous (échelle `stat` ; ex. « Le jeu des champions », « Libre, cadre, bande, 4 billes », « Bientôt sur 1Score »), et une **flèche `→`** en bas à droite qui invite au tap. Tuile BIENTÔT : fond à 45 %, flèche remplacée par le badge `BIENTÔT`, tap sans effet.
...
- **Colonne centrale (état repos)** : titre du mode en surtitre discret (ex. `CADRE 47/2`), puis **une ligne de deux CTA neutres à contour** de demi-largeur : **`CHANGER DE BILLE`** (picto ⇄ sur deux billes) et **`CHANGER DE CÔTÉ`** (picto ⇄ horizontal), puis **`DÉMARRER`** accent, pleine largeur, plus haut qu'un CTA courant (hauteur dans `DESIGN.md` › Buttons). Modèle direct : Cueuny (`공변경` / `자리변경` sur une ligne, `게임시작` en dessous).
...
- **Carte joueur (réorganisée)** — conteneur à contour, fond plein couleur de bille, quatre zones de haut en bas :
  ...
  3. **`MOY` · `SÉRIE`** sur une ligne, sous le score, en petit (échelle `stat`, libellé + valeur, séparés par un point médian). Ils quittent le bandeau.
...
Bille fixe **à la carte** (la couleur suit la bille, jamais le joueur) ; tour actif par **présence** d'un liseré ; zones tactiles ≥ 90×90 px (NFR9) hors claviers (plancher UX-DR8 dans `DESIGN.md` › Keys) ; claviers intégrés, **jamais** le clavier système ; retour haptique + visuel < 100 ms ; validation par `VALIDER` ou 3 s d'inactivité, visible ; « une seule saisie à la fois » ; undo multi-niveaux grisé à pile vide ; pop-ups de décision sans croix, retour toujours nommé `ANNULER` ; règles de fin de partie, égalisatrice, `POUR n`, chrono 40 s en fondu ; aucune alerte d'inactivité ; mise à jour de l'app à l'accueil seulement ; contraste WCAG AA à vérifier sur le bandeau jaune foncé (`#E6B000` + encre noire : conforme — ⚠️ **le token vaut en réalité `--color-panel-yellow-band: #E6CA00`**, calculé pour l'ancien jaune `#FFC72C` ; la carte vaut `#FFE000` depuis la 6e passe de la 10.3. Vérifié au rendu de la 10.4 : le bandeau se distingue encore de la carte, token laissé tel quel) et sur les libellés de picto sur `--color-surface`. *(**Passe contraste AA livrée en Story 10.7, 2026-09-14.** Chiffres figés : encre noire sur `#E6CA00` = **12,81:1** — très largement AA, token laissé tel quel. ⚠️ **Les libellés de picto de la `SideBar` sont sur `--color-sidebar` `#101318`** (**18,61:1**), **pas** sur `--color-surface` : seul `IconAction` est sur `--color-surface` (**16,12:1**). Trois échecs trouvés et corrigés ailleurs : ruban de victoire `#E63946` → `#D0343F` (blanc 4,17 → **4,95:1**), barre de rebours de `ScoreEntryDock` `bg-white/70` → `bg-white` (2,45 → **3,46:1**, objet graphique WCAG 1.4.11), placeholders de `PlayerSetupCard` `opacity-55` → `opacity-60` (4,20 → **4,98:1** sur la carte jaune). Les états `BIENTÔT` à `opacity-45` sont **exclus** de la mesure — WCAG §1.4.3 exempte les commandes inactives, et l'atténuation *est* le signal.)*

### 10.7 — Pictos et libellés

| Action | Picto (trait fin uniforme, `DESIGN.md` › Shapes) | Libellé |
|---|---|---|
