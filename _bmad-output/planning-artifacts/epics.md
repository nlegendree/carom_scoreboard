...
> **Livraison (dev-story, 2026-09-15) — décisions de rendu et écarts.** Rendu statique produit par Playwright headless (l'extension Chrome ne peut pas ouvrir 1920 px sur l'écran de 1440×900 de la machine) et validé par Nathan en cinq passes. **Écart assumé au périmètre « aucune mise en page » (décision de Nathan sur la 3e passe : « en 1920 toutes les proportions ne sont pas bonnes, tout est un peu trop petit ») : la grille `--spacing` devient fluide** (`clamp(8px, 0.678vw, 13px)`) et toutes les tailles de boîte en dérivent (`--size-touch-target` 90 → 146, touches 60/57 → 98/93, `DÉMARRER` 110 → 179, champs, tuiles, débordement du chrono, largeurs de pop-up `max-w-64/72/96`, insets en unités de grille) — ce qui préempte le point « hauteurs en dur → tokens de taille » de la 11.2. Sans cette grille, trois plafonds devaient être bridés (`picto` 13, `label` 30, `stat` 20) parce que la barre latérale de 120 px et les pop-ups de 512 px ne tenaient pas le texte ; ils sont à leur cible (19 / 38 / 23). **CTA de réglage en `label` sur deux lignes** (AC4 par la taille, validé : « c'est bon comme ça »), aucun report du stop clair vers la 11.2. **Rôle `adjust` ajouté** (`−`/`+`, `text-3xl` fixe auparavant). **Bug latent révélé et corrigé** : le demi-anneau de tour de `ShotClock` était clippé depuis le bord du disque ; quand la hauteur gouverne la taille du disque (1920×1080), il est plus étroit que sa zone et 18 px de croissant flottaient dans la colonne — le clip porte désormais sur une couche de la zone. **Constat consigné** : un nom de 11 lettres (`JEAN PIERRE`) se tronque en ellipse dans le champ `NOM` (tablette) et le bandeau du récap (tous formats), comportement antérieur à la story et conforme à la Règle des chiffres nus. Touches alpha re-mesurées : 47 / 50 / 82 px de large (1133 / 1180 / 1920), écart de la 10.7 toujours ouvert sur tablette pour la 11.3. Captures vues et validées, non conservées dans le dépôt (Nathan, 2026-09-16).
...
> **Livraison (dev-story, 2026-09-16).** Décisions de Nathan au rendu, douze passes de captures sans toucher au code : **rayons B** (8 px sur tout tapable, 12 sur la carte de pop-up, 0 sur les conteneurs — `--radius-tappable` / `--radius-popup`), **rouges A** (un seul `--color-brand-red`), **bleu accent A** (retiré, focus rouge vif), **fond : modèle Cueuny** après l'essai du noir pur (« pas assez de contraste ») — deux niveaux de marine `#1E2438` / `#272E49`, barre latérale à ombre portée (`--shadow-sidebar`), cartes de pop-up en verre léger (88 % + flou 8 px, voile sans flou), colonne du scoreboard nue, colonne du paramétrage sans fond ni cadre ; voile 8 %, filet 22 % unique. **Écarts au périmètre, tranchés par Nathan :** `--color-bg` et `--color-surface` bougent, touches et CTA neutres teintés marine, `--gradient-panel` supprimé, six ombres et non cinq. 20 tokens retirés, 9 ajoutés, `tokens.test.ts` verrouille miroir et tokens morts ; `detect` 0 constat ×3 + `src`, 823 tests, build vert, stores intacts. Reports dans `deferred-work.md` (anneau de focus sur tuile bleue 1,37:1 ; cartes arrondies + marges façon Billiboard → 11.3). **Passe de fin, avant code-review** (deux passes de plus) : **bleu roi** `#3B82F6` → `#1D4ED8` sur tout le bleu (le bleu drap de l'Epic 10 sort, `--color-cloth` retiré), `DÉMARRER` en **rouge saturé** `#F04553` → `#C41F2C` détaché du rouge de marque, `PASSER LE TOUR` **bleu** (action de jeu, plus un neutre) ; et la **police** : **Saira** (OFL, variable, auto-hébergée en deux `.woff2`, 58 Ko) retenue parmi six candidates face à `system-ui` — dette « police display » de la 10.1 close, l'échelle 11.1 tient sans retouche.
...
>
> **Six écarts au texte des AC ci-dessus, assumés et documentés** (quatre relevés à la livraison, deux ajoutés à la revue de code du 2026-09-18) :
...
> 5. **Les raisons d'ignore ne citent pas `deferred-work.md` ni `epics.md`** — l'AC3 le demandait mot pour mot ; les huit raisons citent `DESIGN.md` › Écarts assumés et la mesure qui les fonde. **Écart relevé à la revue de code du 2026-09-18**, où il n'était signalé nulle part — le diff avait au contraire réécrit la règle de `DESIGN.md` pour la faire correspondre. Décision de Nathan, même jour : **la nouvelle règle est retenue** (dire ce qui a été MESURÉ), parce que ces huit entrées ne sont pas des éléments de backlog — quatre écarts définitifs, quatre faux positifs du détecteur — et que les faire pointer `deferred-work.md` polluerait la vue de backlog de huit lignes qui ne seront jamais faites. Le lien existe en sens inverse, depuis `deferred-work.md`. Le changement de règle est daté dans `DESIGN.md` › Do / Don't.
> 6. **Nomenclature de sortie des scans : un récapitulatif par lancement**, et non un fichier par mesure comme l'AC1 le reprenait du 2026-09-15 — la première version produisait 37 fichiers par passe, tous contenant exactement `[]` quand l'app est propre. Décision de Nathan du 2026-09-18, consignée dans le Change Log de la fiche mais non remontée ici jusqu'à la revue du même jour.
>
