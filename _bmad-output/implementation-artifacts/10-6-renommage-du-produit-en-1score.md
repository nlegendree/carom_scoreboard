...
- [x] [Review][Patch] *(revue groupée de fin d'Epic 10, 2026-09-15 — corrigé : `LEGACY_GAME_STORAGE_KEY` purgée dans `loadGameState`, test « purges the legacy key on load »)* L'ancienne clé `carom-scoreboard:game` n'est jamais purgée : la garde ne lit que `1score:game`, l'entrée v1 reste orpheline dans le `localStorage` de chaque tablette — le commentaire « l'entrée est supprimée » est faux pour elle [1score/src/services/storageService.ts:12]
