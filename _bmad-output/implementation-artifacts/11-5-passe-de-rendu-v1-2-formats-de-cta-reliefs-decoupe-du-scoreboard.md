...
Seconde revue de code du 2026-09-19 (bmad-code-review, 3 relecteurs sur `2ec8e11..HEAD` — ce qui a suivi la première : jaune, formats de CTA, reliefs, contraste, clôture). 26 constats bruts → 12 retenus après fusion, 11 écartés.

- [x] [Review][Decision] Ombre de barre basse en partie masquée par les cartes — ✅ TRANCHÉ (Nathan, 2026-09-19) : GARDÉE TELLE QUELLE, c'est le rendu validé en « O2 » —  `PlayerPanel` et `CenterPanel` sont `relative`, la rangée de la barre ne l'est pas : les cartes se peignent PAR-DESSUS l'ombre `0 -6px 24px`, visible seulement sur la bande du grand bloc entre barre et cartes. Le rendu livré est bien celui que Nathan a validé en « O2 » (même empilement dans la surcharge). Garder tel quel, ou `relative z-10` sur la rangée pour que l'ombre tombe sur les cartes (changement de rendu → comparaison au rendu d'abord) [1score/src/components/ActionBar.vue:127]
- [x] [Review][Decision] ✅ TRANCHÉ (Nathan, 2026-09-19) : NOUVEAU RÔLE `cta-narrow` (taille de `stat`, 900, interlettré `label`) — frontmatter, `main.css`, puis les deux variantes ; rendu identique vérifié au navigateur (police calculée et largeurs de libellé inchangées aux trois formats). Constat : Frontmatter de `DESIGN.md` : `button-setup` et `button-pass-turn` renvoient à `{typography.stat}` (700, interlettrage `stat` 0,25em) alors que le code rend 900 interlettré `label` — la prose et `design.json` sont justes, le frontmatter (lu par les outils) ne l'est pas. Créer un rôle typographique de CTA étroit (taille `stat`, 900, `tracking.label`) ou documenter l'écart comme surcharge de composant [DESIGN.md:210,226]
- [x] [Review][Patch] Libellé des CTA étroits (`setup`, `pass`) sans garantie d'une seule ligne : l'exception de contraste mesurée à 72,2 % de la hauteur en dépend ; un retour à la ligne remonte le libellé vers le stop clair (3,68:1) sans test rouge — `whitespace-nowrap` dans les deux variantes — sans effet au rendu, vérifié au navigateur : une ligne aux trois formats, ≥ 60 px de marge à 1133 [1score/src/components/CtaButton.vue:80,89]
- [x] [Review][Patch] `DESIGN.md` périmé en quatre endroits : Tuile de mode « bleu sombre à 45 % si BIENTÔT » (sans désaturation ni titre à 50 %) ; Conteneur de récap « libellés blanc 50 % » (75 % depuis R1) ; en-tête d'Elevation « relief réservé … à la barre latérale » (sans la barre basse O2) ; patron des pop-ups « verre léger » (dense depuis V2) [DESIGN.md:453,526,528,544]
- [x] [Review][Patch] « Picto 32 px » des CTA étroits vrai seulement sur tablette : `size-4` est une unité fluide (≈ 52 px à 1920) — `DESIGN.md` › Buttons et le snippet `design.json` (`svg{width:32px}`) [DESIGN.md › Buttons, .impeccable/design.json]
- [x] [Review][Patch] `design.json` en retard : `typographyMeta.label.purpose` range encore les CTA de réglage en `label`, `stat.purpose` ne les cite pas ; description de `button-setup` « picto en ligne » contredite par sa propre suite ; snippets de pop-up sans `:active` (enfoncement) ni `:disabled` (plat) contrairement aux autres CTA ; `flex-shrink:0` du SVG présent sur `setup`, absent sur `pass` [.impeccable/design.json]
- [x] [Review][Patch] `deferred-work.md` : entrées tranchées par la story restées ouvertes — cartes arrondies / ombre de barre basse (O2), verre des pop-ups (V2), découpe Billiboard (livrée), demi-anneau du chrono (composant supprimé) ; entrée `useRejectFeedback` qui renvoie encore à la passe V1.2 close [_bmad-output/implementation-artifacts/deferred-work.md]
- [x] [Review][Patch] Observations de clôture non reportées dans `deferred-work.md` : vérification WebKit / iPad réel jamais faite (flou 16 px, `-webkit-backdrop-filter`, aggravé par V2) ; libellés `stat` des réglages petits à 1920 ; accueil et sélection JDS à angles vifs à côté des écrans en blocs arrondis [_bmad-output/implementation-artifacts/deferred-work.md]
- [x] [Review][Patch] Taille des pictos de réglage non verrouillée : la File List annonce `HomeScreen.test.ts` (Task 4), qui n'a pas été touché — ajouter l'assertion `size-4` (la position du libellé, donc son contraste, en dépend) [1score/src/components/HomeScreen.test.ts]
- [x] [Review][Patch] Test `setup` permissif : `arrayContaining([... 'font-black' ...])` passerait avec un `font-bold` resté à côté — ajouter `not.toContain('font-bold')` [1score/src/components/CtaButton.test.ts:150]
- [x] [Review][Defer] Choix de `PromptModal` à nom long (`${name} JOUE`, 20 caractères, `auto-cols-fr`) : le libellé `accent` sans `min-w-0`/`break-words` déborde du bouton, et l'interlettrage `label` l'aggrave d'environ 10 % [1score/src/components/CtaButton.vue:68, 1score/src/views/GameView.vue:260] — deferred, pré-existant (rejoint l'entrée « Noms longs »)
- [x] [Review][Defer] Exceptions de contraste mesurées (`setup`, `pass`) : `measuredAgainst` ne garde que les arrêts du dégradé, pas la géométrie qui place le libellé (picto, `gap-1`, hauteur tapable) [1score/src/components/CtaButton.contrast.test.ts:149-180] — deferred, outillage
...
- **2026-09-19 — Seconde revue de code, story → `done`.** (bmad-code-review, trois relecteurs sur `2ec8e11..HEAD`). 2 décisions de Nathan (ombre de barre basse gardée telle quelle, « O2 » ; nouveau rôle typographique `cta-narrow` pour les CTA de colonne étroite), 10 correctifs (libellés étroits tenus sur une ligne, `DESIGN.md` et `design.json` réalignés, `deferred-work.md` apuré, deux tests resserrés), 2 reports, 11 constats écartés. 1140 tests, build vert, `design:check:file` à 0.
