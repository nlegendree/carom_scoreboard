...
*Revue de code du 2026-09-18 (`bmad-code-review`), sur le commit `06db3db`. Trois couches — Blind Hunter (patch seul), Edge Case Hunter (patch + projet), Acceptance Auditor (patch + spec + exécution). Chaque constat retenu a été revérifié contre les fichiers réels ; quatre constats de couche ont été rejetés comme faux positifs et ne figurent pas ici.*

**Décisions prises (2026-09-18, Nathan)** — les deux sont tranchées et appliquées ; elles sont devenues des correctifs de documentation.

- [x] [Review][Decision] **Aucune des huit raisons d'ignore ne cite `deferred-work.md` ni `epics.md`, et `DESIGN.md` a été modifié pour retirer l'exigence** — l'AC3 (et `epics.md:2113`) demande mot pour mot une raison « qui cite l'entrée de `deferred-work.md` ou la décision d'`epics.md` ». `grep -c "deferred-work\|epics.md" .impeccable/config.json` rend **0** : les huit raisons citent `DESIGN.md › Écarts assumés` et « Story 10.3 ». Les raisons sont substantielles et mesurées (482 à 647 caractères) — l'esprit est tenu. Mais le diff modifie aussi la règle de `DESIGN.md` (« en citant `deferred-work.md` » → « en disant ce qui a été MESURÉ »), c'est-à-dire qu'il **aligne la règle sur ce qui a été fait** au lieu de nommer l'écart. C'est le **cinquième** écart au texte des AC, et `epics.md` en annonce quatre (six avec la nomenclature de sortie du Change Log). Deux issues : (a) ajouter la citation aux huit raisons et restaurer la phrase de `DESIGN.md` ; (b) assumer l'écart et le signaler comme les quatre autres. Ce qui n'est pas tenable, c'est de réécrire la règle en silence.
  → **Tranché par Nathan le 2026-09-18 : la nouvelle règle est retenue.** Motif donné : `deferred-work.md` lui sert de **vue de backlog**, et ces huit entrées n'en sont pas — quatre écarts définitifs, quatre faux positifs du détecteur ; les y faire pointer aurait ajouté huit lignes qui ne seront jamais faites. Le lien est donc posé **en sens inverse** (`deferred-work.md` porte une entrée sur ce jeu d'ignores et sur ce qui le rouvrirait). Appliqué : changement de règle daté dans `DESIGN.md` › Do / Don't, et **cinquième écart** consigné dans `epics.md` — où un **sixième** a été ajouté au passage (la nomenclature de sortie du Change Log, qui n'y avait jamais été remontée).
- [x] [Review][Decision] **Divergence de format : `CLAUDE.md` §9 exige l'iPad 11″ en 1194×834, tout l'outillage scanne 1180×733** — `design-check.sh:41`, `render-static.cjs`, `CtaButton.contrast.test.ts:800-805` et les trois lignes de base utilisent 1180×733, et le commentaire neuf de la story écrit `// Les trois formats du produit (CLAUDE.md §9)` juste au-dessus de `{ name: '1180×733 (iPad 11″)' }` — en citant §9 pour un chiffre que §9 contredit. Les deux nombres désignent deux tablettes : 1194×834 est l'iPad Pro 11″, 1180×820 l'iPad Air 11″ (733 = 820 moins le chrome Safari, donc une mesure réelle). 1180×733 préexiste à la story, mais celle-ci l'institue en garde-fou et scelle la contradiction. Il faut trancher quelle tablette est la cible, puis aligner `CLAUDE.md` §9 ou l'outillage — l'écart est de 14 px de large et **101 px de haut**, et aucun des 36 scans ne couvre l'intervalle.
  → **Tranché par Nathan le 2026-09-18 : la cible est l'iPad Air 11″, donc 1180×733 est le bon chiffre**, et pas de re-scan. Appliqué : `CLAUDE.md` §9 corrigé (« iPad 11″ 1194×834 » → « iPad Air 11″ 1180×733 »), et §8 reçoit la note qui dit d'où vient le chiffre (1180×820 points moins le chrome Safari) et pourquoi il ne faut pas le « corriger » en 1194×834, qui est l'iPad **Pro**. Les trois lignes de base restent valides.

**Correctifs — les 16 sont APPLIQUÉS** (revue du 2026-09-18). Validation après correction : `npm test` **1107 / 32 fichiers** vert, `npm run build` vert, `src/stores/` toujours intact, absence du build reconfirmée sur sept identifiants, `npm run design:check` rejoué sur serveur neuf → **36/36 propres, exit 0**, `design:check:file` propre. Aucun fichier de l'application n'a été modifié : les correctifs portent sur les tests, le script de scan, le lecteur de frontmatter et la documentation.

- [x] [Review][Patch] **Tout code de sortie ≠ {0,1,2} se lit « aucun constat (exit 0) », et le récapitulatif n'est pas écrit** [1score/scripts/design-check.sh:96,118-137] — `note() { case "$1:$status" in 1:*) status=1 ;; 2:0) status=2 ;; esac; }` ne retient l'échec que pour le code **exactement 1**. Vérifié : `127 → 0`, `139 → 0`, `3 → 0`. Un binaire tué, un Chromium en segfault, un `bad CPU type` (le cas « second poste » qu'invoque l'AC8) affichent `ÉCHEC DE SCAN` dans le corps et ressortent `→ aucun constat (exit 0)`. Aggravant : la branche d'échec journalise le fichier **`.err`**, mais `write_run_file` ne traite `entry.error` que pour `code === 1` — pour 127 il `JSON.parse` le message d'erreur, node meurt (`SyntaxError: Unexpected token 'E'`), **aucune archive n'est écrite**, et le script imprime quand même `archivé : …`. Rejoué trois fois de façon indépendante. C'est la promesse centrale de l'AC1 (« un script qui avale le `2` ne garde-fou rien ») qui tombe. Correctif : motif par défaut `*) status=1` (en excluant 0 et 2), branche `error` sur tout code ≠ 0 et ≠ 2, et contrôle du code de retour de `write_run_file`.
- [x] [Review][Patch] **Le test de la garde de production est un placebo : retirer la garde laisse le test vert** [1score/src/dev/scenes.test.ts:104] — `source.indexOf('import.meta.env.DEV')` prend la **première occurrence textuelle**, qui est un **commentaire** dans deux des trois fichiers (`main.ts:27`, `HomeScreen.vue:141` ; `GameView.vue` n'échappe que par accident, son commentaire ne contient pas la chaîne). Prouvé par mutation : garde supprimée de `main.ts`, commentaire laissé → `guard=1254 < call.index=1408` → **TEST VERT**, et la table des douze scènes part dans `dist/`. Dans une story dont la thèse est « un garde-fou qui ne casse pas quand on casse le code ne prouve rien », c'est le garde-fou le plus important qui ne casse pas. Correctif : chercher le littéral `if (import.meta.env.DEV)`, ou dépouiller les commentaires avant de chercher. À traiter aussi : le test voisin `:109` (`not.toMatch(/^(?!.*⚠️).*if\s*\(import\.meta\.env\.DEV/m)`) n'exempte aucune ligne réellement présente — garde morte.
- [x] [Review][Patch] **Deux des trois listes d'exclusion de l'AC5 sont restées écrites à la main** [1score/src/assets/typography.test.ts:44 ; tokens.test.ts:21] — `const SPACING_STEPS = ['base','xs','sm','md','lg','xl','2xl']` et `const TAILWIND_RADII = ['none','full']` sont des recopies littérales du frontmatter, sous un commentaire qui revendique « Deux exclusions … qui se DÉRIVENT elles aussi », et le Dev Agent Record écrit « les six listes sont dérivées, **exclusions comprises** ». Mutations rejouées : retirer `spacing: md` de `DESIGN.md` → **238 tests verts**, retirer `rounded: full` → **179 tests verts**. Le miroir a donc un trou exactement là où l'AC5 le visait. Seule `VIEWPORT_CLAMPED_ROLES` dérive authentiquement. Correctif minimal et honnête : exiger que chaque nom exclu **existe encore** dans le frontmatter, ce qui rend le retrait rouge sans prétendre dériver l'indérivable ; et corriger les deux affirmations.
- [x] [Review][Patch] **Le garde-fou du « vocabulaire unique » ne lit jamais le harnais** [1score/src/dev/scenes.test.ts:26] — `HARNESS_NAMES` est recopié à la main, sous un commentaire qui dit « recopiés ici À LA MAIN et volontairement : c'est le seul endroit qui vérifie que le vocabulaire … n'a pas divergé ». `render-static.cjs` n'est lu par aucun test : renommer `shot(page, f, '11-recap')` en `'11-recapitulatif'` ne fait rien rougir. Les douze noms coïncident aujourd'hui (vérifié) — c'est la garde qui manque, pas l'état. Le test d'à côté lit déjà `design-check.sh` par `import.meta.glob('../../scripts/design-check.sh', { query: '?raw' })` : la même ligne sur `render-static.cjs` ferme le trou.
- [x] [Review][Patch] **Le parseur de frontmatter est muet sur cinq formes sur six** [1score/src/assets/designFrontmatter.ts:44-87] — exécuté contre le module réel : indentation par **tabulation** → `{"none":"0px"}` (deux entrées avalées) ; **commentaire de fin de ligne** (`tappable: 8px # rayon`) → valeur corrompue `"8px # rayon tapable"` ; **clé dupliquée** → la dernière gagne ; **section dupliquée** → la seconde ignorée ; **commentaire en colonne 0** au milieu d'une section → `break`, troncature. Seule l'indentation à espaces non conforme lève. L'en-tête du fichier promet « mieux vaut le dire que rendre une liste silencieusement incomplète » : c'est vrai d'un cas sur six. ⚠️ **Portée réelle, à ne pas gonfler** : le miroir à double sens (`typography.test.ts:74,103`) compare les listes dérivées à ce que `main.css` déclare, donc toute troncature qui **retire un rôle** fait diverger les deux ensembles et rougit. Le silence ne subsiste que sur les formes qui **corrompent une valeur** sans changer la liste (commentaire de fin de ligne, clé dupliquée). Correctif de quelques lignes : refuser la tabulation, refuser un `#` dans une valeur non guillemetée, refuser une clé ou une section dupliquée, ne pas `break` sur un commentaire.
- [x] [Review][Patch] **L'exception mesurée `pass` est un cliquet à sens unique** [1score/src/components/CtaButton.contrast.test.ts:226-233 et 188-206] — dans la branche `if (exception)`, `stops` n'est **jamais lu** : les trois `it` comparent le littéral `5.18` (mesure du 2026-09-17) à un seuil calculé, lui, depuis la source. Éclaircir `--gradient-blue` dégrade le fond réel sous le libellé sans que rien ne rougisse, et le test « exception devenue inutile » ne se déclenche que dans l'autre sens (il exige `failsSomewhere === true`, ce qu'un dégradé plus clair rend **plus** vrai). ⚠️ Le sens qui marche a été vérifié : monter `pass` en `label` fait bien rougir avec le message « retirer l'exception » — l'auto-retrait promis fonctionne. C'est la dégradation qui n'est pas couverte. Correctif : enregistrer dans `MEASURED_ON_SURFACE` les stops contre lesquels la mesure a été prise, et rougir si `gradientStops()` ne les rend plus.
- [x] [Review][Patch] **« Graisses lues dans le frontmatter de `DESIGN.md` » est inexact, et `DESIGN.md` se contredit sur la graisse de `stat`** [1score/src/components/CtaButton.contrast.test.ts:160 ; DESIGN.md:385,463] — les tailles viennent bien du frontmatter, les graisses viennent des classes Tailwind (`font-black ? 900 : font-bold ? 700 : 400`), avec repli muet à 400. L'affirmation figure telle quelle dans le Dev Agent Record, dans `DESIGN.md` › Écarts assumés et dans `CLAUDE.md` §9 : trois endroits à corriger. Par ailleurs `DESIGN.md:463` écrit « à **14 px/900** le seuil est 4,5:1 » alors que `:385` déclare `stat` en **700** — la conclusion ne bouge pas (à 14 px on est sous le plancher 18,66 px dans les deux cas, seuil 4,5:1), mais le chiffre est faux.
- [x] [Review][Patch] **Un identifiant de scène inconnu, ou une liste vide, scanne l'accueil et rend « propre »** [1score/scripts/design-check.sh:57-64,145-150 ; 1score/src/dev/scenes.ts:218-222] — `--scene 07-scoreboard-JDS` (casse), `--scene 07-scoreboard-jd` (frappe) ou un renommage non répercuté : `readScene()` rend `null`, l'accueil s'affiche, le détecteur mesure l'accueil, exit 0. Et `--scene ""` ou `--viewport ""` ne fait **aucune** mesure : `scans: 0, findings: 0, exitCode: 0`. C'est le placebo décrit en tête de `scenes.ts` (« mesurerait douze fois l'accueil en croyant mesurer le scoreboard »), déplacé d'un cran vers la ligne de commande. Correctif : le script refuse une valeur de `--scene` absente de sa propre liste, et refuse une liste vide.
- [x] [Review][Patch] **Une option en dernier argument tue le script avec un message illisible et un code hors convention** [1score/scripts/design-check.sh:39,58-60] — `npm run design:check -- --url` : `set -u` fait avorter sur `$2: unbound variable`, aucun des trois messages finaux n'est imprimé, et le code de sortie est celui du shell (vérifié : **1** ici ; **2** sous dash, c'est-à-dire « des constats » dans la convention du script). C'est exactement le plantage que l'AC8 vient de fermer côté `render-static.cjs` (`--url` / `--override` en dernier argument) et qui renaît dans le script neuf de la même story. Correctif : `[ $# -ge 2 ] || { echo "…attend une valeur" >&2; exit 1; }`.
- [x] [Review][Patch] **Un lancement partiel écrase le récapitulatif du jour et se fait passer pour lui** [1score/scripts/design-check.sh:79] — `RUN_FILE="$BASELINE_DIR/$DATE-scan-$MODE.json"` ne porte ni l'heure ni la portée. Vérifié : run complet → `scans=36`, puis `--scene 01-accueil --viewport 1920x1080` le même jour → `scans=1`, même nom, relevé des 36 mesures perdu. Le JSON n'enregistre ni la liste des scènes ni celle des formats effectivement scannés : relu plus tard, `"scans": 1` est indistinguable d'un scan complet qui a planté après la première mesure. Correctif minimal : écrire `scenes` et `viewports` dans le récapitulatif — un lancement partiel se dénonce alors tout seul.
- [x] [Review][Patch] **Trois formes que le lecteur de variantes accepte sans savoir les lire** [1score/src/components/CtaButton.contrast.test.ts:85,156,160] — (a) `gradientStops` sur `/#[0-9a-fA-F]{6}/g` tronque un hexadécimal à 8 chiffres : `#3B82F6CC` est lu `#3B82F6`, alpha jeté, ratio calculé contre un fond qui n'existe pas — alors qu'une notation `rgb()` ou `color-mix()` jette bruyamment ; (b) `/\btext-(white)\b/` matche `text-white/90`, l'opacité disparaît (à 80 % sur le bleu, le vrai ratio tombe à ~2,5:1 contre 3,68 calculé) ; (c) le rôle retenu est le **premier** `text-<x>` reconnu, donc un futur `max-lg:text-stat` posé avant `text-label` serait ignoré. Aucune de ces formes n'existe dans `VARIANT_CLASSES` aujourd'hui — ce sont trois silences latents dans le test qui porte désormais seul le contraste des six CTA.
- [x] [Review][Patch] **Une taille non résoluble fait planter le test de contraste au lieu de l'échouer** [1score/src/components/CtaButton.contrast.test.ts:223,229] — `sizeAtWidth(size, width)!` rend `null` sur un `cqw`, un `min(…)`, un `rem` ou un `vh` (formes déjà présentes ailleurs dans le frontmatter : `typography.series`, les cinq `score-*`). `threshold(null, w)` rend alors 4,5 en silence, puis le message d'`expect` évalue `px.toFixed(1)` → `TypeError: Cannot read properties of null`. Le contrôle dédié (`:179-186`) attrape bien la condition avec un message propre, mais les 18 `it` du cœur plantent en parallèle avec une trace qui ne nomme pas la cause.
- [x] [Review][Patch] **`clamp()` évalué dans l'ordre inverse de la spéc CSS quand `min > max`** [1score/src/assets/designFrontmatter.ts:124] — le code fait `Math.min(Math.max(preferred, min), max)` ; CSS Values 4 définit `clamp(MIN, VAL, MAX)` comme `max(MIN, min(VAL, MAX))`. Vérifié : `clamp(38px, 2vw, 20px)` à 1180 rend **20** ici, **38** au navigateur. Inoffensif tant que `min ≤ max` partout, et rien ne le vérifie. Un écart de 18 px fait basculer le seuil WCAG de 3:1 à 4,5:1. Correctif d'une ligne.
- [x] [Review][Patch] **Une `--url` qui porte déjà une query produit une URL malformée** [1score/scripts/design-check.sh:148] — `--url 'http://localhost:5174/?dev=1'` donne `http://localhost:5174/?dev=1/?scene=07-scoreboard-jds` : Vite sert l'index, `readScene()` rend `null`, l'accueil est scanné, exit 0. Se replie sur le constat « scène inconnue ».

**Reports (non traités ici)**

- [x] [Review][Defer] **Lancer `design:check` efface la partie sauvegardée réelle du développeur** [1score/src/dev/scenes.ts:228-231] — reporté
- [x] [Review][Defer] **`keepSceneOutOfStorage` repose sur `pagehide`, que le store voisin documente comme non fiable, et ne couvre pas l'onglet concurrent** [1score/src/dev/scenes.ts:255-258] — reporté
- [x] [Review][Defer] **`?scene=` re-sème `HomeScreen` à chaque remontage** [1score/src/components/HomeScreen.vue:137-156] — reporté
- [x] [Review][Defer] **Le contenu des scènes n'est confronté à rien : seuls les noms sont verrouillés** [1score/src/dev/scenes.test.ts:43-60] — reporté
- [x] [Review][Defer] **Les huit ignores sont `value: "*"` : la règle tombe entièrement sur la scène visée** [.impeccable/config.json] — reporté
- [x] [Review][Defer] **L'archive des 36 scans porte `findings: 0`, les 24 constats bruts n'ont pas de trace machine** [.impeccable/baseline/2026-09-18-scan-url.json] — reporté
- [x] [Review][Defer] **`render-static.cjs` : `--override <dossier>` jette un `EISDIR` nu, `1920x1080x5` est accepté en silence** [1score/scripts/render-static.cjs:56-60,74-79] — reporté, hors des cinq cas bornés par l'AC8
- [x] [Review][Defer] **`MODULE_NOT_FOUND` d'une dépendance transitive est attribué à Playwright lui-même** [1score/scripts/render-static.cjs:37-40] — reporté

**Rejetés comme faux positifs** — `VIEWPORT_CLAMPED_ROLES` qui rétrécirait en silence (le miroir à double sens de `typography.test.ts:103` fait diverger les ensembles et rougit) ; la raison du lanceur absente de `package.json` (la Task 2 autorisait explicitement « ou `README` », et JSON ne porte pas de commentaire) ; « `PARTIE EN COURS` n'aurait pas gêné une scène 01–06 » (prémisse fausse : `GameView.vue:346-355` rend la pop-up par-dessus `HomeScreen` quand `status === 'idle'` — le `discardSavedGame()` en tête de scène est nécessaire aux douze) ; la mutation de l'ordre `checkSavedGame` → `applyStoreScene` → `app.mount()` (vérifiée correcte).

**Ce que la revue a vérifié et trouvé réellement tenu** — `npm test` **1095 / 32 fichiers** et `npm run build` verts, chiffres exacts ; `src/stores/` intact ; absence du build confirmée sur sept identifiants, y compris insensible à la casse ; les douze `SCENE_IDS` identiques aux douze `shot()` du harnais ; scène posée avant `app.mount()`, semis au `setup` ; `npm run design:check` rejoué → 36/36 propres, archive reproduite **à l'octet** ; les ignores sont porteurs et non décoratifs (scan sans `config.json` → exit 2 sur les scènes 03/07/08/10) ; zéro `ignoreRules`, zéro `ignoreFiles` ; les quatre mutations d'AC4 et les quatre d'AC5 rejouées et rouges comme annoncé ; `DESIGN.md` corrigé aux deux endroits ; `.impeccable/design.json` non touché et `rejectKey` présent dans exactement trois fichiers ; « 0 rôle ne plafonne avant 1280 px » recalculé et exact (premier plafonnement à 1900 px). **L'AC4 est le meilleur travail de la story** : la mutation échouée est dite en clair, la cause isolée, et la compensation fonctionne réellement.
## Dev Notes
...
**Conséquence tirée, plutôt que tue** : `1score/src/components/CtaButton.contrast.test.ts` refait le calcul **depuis la source** — stops lus dans `main.css`, tailles lues dans le frontmatter de `DESIGN.md` (même source unique que l’AC5) et graisses lues dans les classes Tailwind de `CtaButton.vue` — ⚠️ pas dans le frontmatter, correction de la revue du 2026-09-18, seuil WCAG (24 px, ou 18,66 px en ≥ 700) appliqué aux trois formats. Règle **conservatrice** : le libellé doit tenir le seuil contre l'arrêt **le plus clair**, donc où qu'il se pose. Vérifié par mutation :
