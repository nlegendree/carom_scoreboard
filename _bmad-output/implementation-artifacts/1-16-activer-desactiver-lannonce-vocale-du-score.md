...
| `src/services/storageService.ts` (~110 l.) | clé `carom-scoreboard:game`, enveloppe versionnée, `isRecord`/`parseJson`, AR12 | `+ SETTINGS_STORAGE_KEY/VERSION`, `loadSettings`, `saveSettings` |
...
- Nouveaux : `carom-scoreboard/src/types/settings.ts`, `src/stores/useSettingsStore.ts`, `src/stores/useSettingsStore.test.ts`, `src/composables/useSpeech.ts`, `src/composables/useSpeech.test.ts`, `_bmad-output/implementation-artifacts/1-16-actions-manuelles-nathan.md`.
...
- [Source: 1-12 story — `GameState` = forme persistée versionnée, clé `carom-scoreboard:game`, garde structurelle minimale, `console.warn` sur sauvegarde illisible]
...
- [Source: carom-scoreboard/CLAUDE.md §2 (pointer), §3 (storage en service), §4 (Pinia, `storeToRefs`), §6 (tests co-localisés), §7 (`--spacing: 8px`), §9 (validation)]
